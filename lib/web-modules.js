...
function useWebModules(config = loadWebModulesConfig()) {
    ...
    async function taskPlugins(module, filename) {
        if (filename) {
            if (squash(module)) {
    ...
    async function isEsModule(module) {
        return new Promise(function (done, fail) {
            resolve_1.default(`${module}/package.json`, { moduleDirectory: config.resolve.paths }, function (err, resolved, pkg) {
                var _a;
                if (pkg) {
                    done(pkg.module || pkg["jsnext:main"] || ((_a = pkg.main) === null || _a === void 0 ? void 0 : _a.endsWith(".mjs")));
                }
                else {
                    fail(err);
                }
    ...
    function rollupWebModule(pathname) {
        ...
        async function rollupWebModuleTask(module, filename) {
            tiny_node_logger_1.default.info("rollup web module:", module, "filename:", filename);
