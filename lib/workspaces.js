...
function readManifest(basedir, entries = []) {
    try {
        ...
        entries.push([pkg.name, path_1.default.join(basedir, mainFile(pkg))]);
        if (pkg.workspaces) {
            tiny_node_logger_1.default.info("loading workspaces from:", pkg.name);
        ...
        tiny_node_logger_1.default.info("no package.json found at:", basedir);
    }
    return entries;
}
